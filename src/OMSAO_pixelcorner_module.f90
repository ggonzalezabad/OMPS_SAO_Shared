...
CONTAINS
  ...
  SUBROUTINE compute_pixel_corners ( ntimes, nxtrack, lat, lon, errstat )
    ...
    CALL sphgeo_comp_pixel_corners (                                            &
         nXtrack, ntimes, lon(1:nXtrack,0:ntimes-1), lat(1:nXtrack,0:ntimes-1),  &
         corner_lon(0:nXtrack,0:ntimes), corner_lat(0:nXtrack,0:ntimes), estat, &
         yn_omi_pixel_adjust_k=.FALSE. )

    ! --------------------------------------
    ! For OMPS write pixel corners in one go
    ! --------------------------------------
    ! ---------------------------------------------
